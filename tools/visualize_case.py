...
def viz_client_metrics(sim_run):
    ...
    for metric in ["read", "write"]:
        ...
        has_data = not throughput.empty and metric in throughput.columns and (throughput[metric] > 0).any()
        ...
        if has_data:
            ...
            plt.title(f"Client {metric.capitalize()} Throughput Over Time (Smoothed {window_size}s) - {case_name}")
            if metric == "read":
                plt.ylim(bottom=0)
            plt.legend()
        else:
            plt.title(f"Client {metric.capitalize()} Throughput (No Data) - {case_name}")
...
def viz_net_metrics(sim_run):
    ...
    plt.title(f"Network Topology ({case_name})", pad=10)
    plt.axis("off")
